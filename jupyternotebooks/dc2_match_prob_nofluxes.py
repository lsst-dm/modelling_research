...
butler = dafButler.Butler(
    ...
    collections=[
        '2.2i/truth_summary',
        '2.2i/runs/test-med-1/w_2021_36/DM-31699',
    ]
)
...
path_match = '/project/dtaranu/dc2_gen3/w_2021_36_match/'
cats_in, cats_out = {}, {}
    ...
    for tract in tracts:
        ...
        if write:
            pq.write_table(pa.Table.from_pandas(catalog_out_ref),
                           f'{path_match}match_{model_target}_ref_{tract}{postfix}.parq')
            pq.write_table(pa.Table.from_pandas(catalog_out_target),
                           f'{path_match}match_{model_target}_target_{tract}{postfix}.parq')
else:
    for tract in tracts:
        cats_out[tract] = (
            pq.read_table(f'{path_match}match_{model_target}_ref_{tract}{postfix}.parq').to_pandas(),
            pq.read_table(f'{path_match}match_{model_target}_target_{tract}{postfix}.parq').to_pandas(),
        )
...
_ = plot_matches(
    ...
    limits_y_color=lim_y['unresolved'],
    **kwargs
)
