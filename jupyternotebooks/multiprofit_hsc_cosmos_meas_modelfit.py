...
for bands in ['i', 'griz', 'iz']:
    files = np.sort(glob.glob(f"{path_proj}2019-11-[0-3][0-3]/hsc_{bands}/mpf_9813_[0-9],[0-9]_{bands}_mag.fits"))
    tables = []
    patches = []
    patch_rows = []
    rows = 0
...
cats['f814w'] = vstack([SourceCatalog.readFits(f'{path_proj}/2019-11-12/hst_iso/mpf_9813_{x}_F814W_iso.fits').asAstropy() for x in patches])
...
def plot_mpf_model_hsc_vs_hst(model):
    ...
    good = cond & np.isfinite(flux_hsc) & np.isfinite(flux_hst)
    y = np.log10(flux_hsc[good]/flux_hst[good]) - 0.88
    x_good = x[good]
    n_good = np.count_nonzero(good)
    if is_single_comp:
        reff_hst = np.log10(data_hst["reff_1"][good]*0.03)
        reff_hsc = np.log10(data_hsc["reff_1"][good]*0.168)
...
# Plot hopefully more robust exponential fits
