...
class MultiProFitConfig(pexConfig.Config):
    ...
    bands = pexConfig.ListField(dtype=str, default=[], doc="List of expected bandpass filters.",
    ...
    disableNoiseReplacer = pexConfig.Field(
        dtype=bool, default=False, doc="Whether to disable replacement of nearby sources with noise when fitting "
                                       "deblended single sources")
    ...
    field_localbg = pexConfig.Field(
        dtype=str, default='base_LocalBackground', doc="Field name (prefix to _instFlux) to read the local background "
                                                       "level from if usePriorBackgroundLocalEstimate is True")
...
class MultiProFitTask(mrFitmb.MultibandFitSubTask):
    ...
    def _getExposurePsfs(
            self, exposures, source, extras, footprint=None, failOnLargeFootprint=False, idx_src=None, idx_mask=None
    ):
        ...
        if fit_hst:
            ...
            for extra, (band, exposure), img_psf in zip(extras, exposures.items(), img_psfs):
                ...
                # TODO: Reassess whether this is necessary (the float cast to copy, not the subset(bbox).array)
            ...
            if not deblend_from_fits:
                ...
                if (not self.config.plotOnly) and self.config.fitGaussian:
                    ...
                    result = self.__fitModel(model, exposurePsfs, modeller=self.modeller, sources=sources,
                                             resetPsfs=True, plot=self.config.plot and len(self.modelSpecs) == 0)
                    results['fits']['galsim'][name_modeltype] = mpfFit.ModelFits(fits=[result], modeltype='gaussian:1')
                ...
                for modelSpec in self.modelSpecsDict.values():
                    ...
                    if n_good > 0:
                        if self.config.plotOnly:
                            ...
                            result_model, _ = mpfFit.fit_model(model, plot=self.config.plot, kwargs_fit=kwargs_fit)
    ...
    def __setFieldsPsf(self, results, fieldsBase, fieldsExtra, row, key_index=None):
        ...
        if results_psfs:
            for idxBand, band in enumerate(self.config.bands):
    ...
    def fit(
        ...
        **kwargs
        ...
        for catexp in catexps:
            if catexp.band not in self.config.bands:
                raise ValueError(f'catexp band={catexp.band} not in self.config.bands={self.config.bands}')
            if catexp.band in catexps:
                raise ValueError(f'catexp band={catexp.band} already found; duplicates are not supported')
            data[catexp.band] = catexp
        bands = data.keys()
        if set(bands) != set(self.config.bands):
            raise RuntimeError(f'bands={bands} not identical to config.bands={self.config.bands}')
        exposures = {}
        bbox_ref = None
        ...
        if self.config.pathCosmosGalsim is not None:
            ...
            if self.config.disableNoiseReplacer:
                ...
                for exposure_extra, sources_extra, meta_extra in extras:
                    ...
                    for idx_extra, source_extra in enumerate(sources_extra):
                        ...
                        is_blended = (cat_ref.find(parent_extra) if has_parent else source_extra)['deblend_nChild'] >= 1
                        # Add parents back in when fitting, except if they have only one child
                        # Parents with one child don't need to be subtracted or added back in since they're not blended
                        idx_add[idx_extra] = [] if not is_blended else (
                            [idx_extra] if has_parent else [
                                int(idx_child) for idx_child in np.where(cat_ref['parent'] == source_extra['id'])[0]
                            ]
                        )
            ...
            if self.config.bboxDilate > 0:
                extras = []
                for datum in data.values():
                    extras.append(MultiProFitTask._getSegmentationMap(datum.exposure.getBBox(), datum.catalog))
            else:
        ...
        if self.config.idx_begin < 0:
            self.config.idx_begin = 0
        if self.config.idx_end > numSources or self.config.idx_end < 0:
            self.config.idx_end = numSources
        numSources = self.config.idx_end - self.config.idx_begin
        ...
        if not init_from_cat:
            src = cat_ref[int(np.where(cat_ref['parent'] != 0)[0][0])]
            results, error, deblended = self.__fitSource(src, exposures, extras, skip_fit=True)

            # If one of the models failed, we can't set up the catalog from it
            if not all([isinstance(x, mpfFit.ModelFits) for x in results['fits']['galsim'].values()]):
                raise RuntimeError('Not all fits passed')
            catalog, fields = self.__getCatalog(bands, results, catalog_in)

        deblend = self.config.deblend or self.config.deblendFromDeblendedFits

        for idx in range(np.max([self.config.idx_begin, 0]), self.config.idx_end):
            src = cat_ref[idx]
            results = None
            ...
            if not skipped:
                ...
                id_src = src['id']
                # Scarlet/meas_extensions_scarlet has isolated children; meas_deblender does not
            ...
            if deblended:
                if self.config.disableNoiseReplacer:
                    # This isn't actually necessary as long as the exposures are float64 copies, as they are now
    ...
    def get_schema(self, schema_in) -> afwTable.Schema:
        ...
        results = mpfFit.fit_galaxy_exposures(
            exposurePsfs, self.config.bands, self.modelSpecs, skip_fit=True, skip_fit_psf=True,
        )
        catalog, fields = self.__getCatalog(self.config.bands, results, afwTable.SourceCatalog(schema_in))
        return catalog.schema

    def get_bands_calib_required(self):
        return set()
